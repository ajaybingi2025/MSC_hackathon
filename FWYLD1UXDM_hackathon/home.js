...
// js of info page
// Get the button to assign tasks
const assignTasksButton = document.getElementById("assignTasksButton");

// Add event listener to assign tasks button
assignTasksButton.addEventListener("click", assignTasks);

// Function to assign tasks to members using round-robin algorithm
function assignTasks() {
    const unassignedTasks = document.querySelectorAll('.todo-list .not-completed p');
    const membersCount = members.length;
    let memberIndex = 0;

    unassignedTasks.forEach(task => {
        // Assign task to a member using round-robin
        const member = members[memberIndex];
        assignTaskToMember(task.textContent, member.name);

        // Update member index for next task
        memberIndex = (memberIndex + 1) % membersCount;
    });
}
